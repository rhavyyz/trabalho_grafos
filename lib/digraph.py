...
class Digraph:
    ...
    def add_edge(self, source, destination, weight=1):
        ...
        """
        if self.weighted:
            self.adjacency_list[source - 1].append((destination, weight))
        else:
            self.adjacency_list[source - 1].append((destination, 1))
    ...
    def maxd(self):
        """
        Retorna o grau de saída máximo no dígrafo.
    ...
    def mind(self):
        """
        Retorna o grau de saída mínimo no dígrafo.
    ...
    def w(self, source, destination):
        ...
        """
        for neighbor, weight in self.adjacency_list[source - 1]:
    ...
    def bfs(self, source):
        ...
        predecessors = [-1] * self.n  # Predecessores no caminho

        queue = Queue()
        queue.put(source)
        visited[source - 1] = True
        distances[source - 1] = 0
        predecessors[source - 1] = None  # O próprio vértice fonte não tem predecessor

        # Enquanto a fila não estiver vazia
        while not queue.empty():
            # Retira o primeiro elemento da fila
            current_vertex = queue.get()
            for neighbor, _ in self.adjacency_list[current_vertex - 1]:
                if not visited[neighbor - 1]:
                    visited[neighbor - 1] = True
                    distances[neighbor - 1] = distances[current_vertex - 1] + 1
                    predecessors[neighbor - 1] = current_vertex
                    queue.put(neighbor)
    ...
    def dfs(self, source):
        ...
        visited = [False] * self.n # Vértices visitados
        predecessors = [-1] * self.n # Predecessores no caminho
        discovery_time = [-1] * self.n # Tempos de descoberta
        finish_time = [-1] * self.n # Tempos de término

        # Entry - Visita o vértice pela primeira vez
        # Exit - Visita o vértice pela segunda vez
        stack = [(source, 'entry')] # Pilha de vértices a serem visitados
        time = 0
        ...
        while stack:
            ...
            if action == 'entry':
                ...
                discovery_time[vertex - 1] = time
                time += 1
                stack.append((vertex, 'exit'))

                # A diferença chave: apenas os vizinhos de saída são considerados
                for neighbor, _ in self.adjacency_list[vertex - 1]:
                    if not visited[neighbor - 1]:
                        predecessors[neighbor - 1] = vertex
